...
plot_partial_pooling <- function(df, y_var, x_var, group) {
    ...
    df <- dplyr::mutate(df, year = as.character(!!group_enquo))
    
    print(df)
    
    not_pooled_formula <- stats::as.formula(paste(y_var_q, " ~ ", x_var_q, " | ", group_q, sep = ""))
    ...
    x_var_enquo <- dplyr::enquo(x_var)
    y_var_enquo <- dplyr::enquo(y_var)
    ...
    pooled_formula <- stats::as.formula(paste(y_var_q, " ~ ", x_var_q, sep = ""))
    m_pooled <- stats::lm(pooled_formula, data = df)
    ...
    partially_pooled_formula <- stats::as.formula(paste(y_var_q, " ~ ", x_var_q, " + (1 |", group_q, ")", sep = ""))
